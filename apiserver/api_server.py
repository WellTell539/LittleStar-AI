...
async def get_emotional_status():
    ...
    try:
        if hasattr(naga_agent, 'emotional_ai') and naga_agent.emotional_ai:
            status = naga_agent.emotional_ai.get_emotion_status()
            return {"status": "success", "data": status}
        else:
            return {"status": "success", "data": {"status": "disabled", "message": "情绪AI系统未启用"}}
    except Exception as e:
        print(f"获取情绪状态错误: {e}")
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"获取情绪状态失败: {str(e)}")

@app.post("/emotional/thinking")
async def trigger_thinking():
    ...
    try:
        if hasattr(naga_agent, 'emotional_ai') and naga_agent.emotional_ai:
            message = naga_agent.emotional_ai.generate_proactive_message()
            return {"status": "success", "data": {"message": message}}
        else:
            return {"status": "error", "message": "情绪AI系统未启用"}
    except Exception as e:
        print(f"触发AI思考错误: {e}")
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"触发AI思考失败: {str(e)}")

class EmotionRequest(BaseModel):
    emotion_type: str
    intensity: float = 0.5

@app.post("/emotional/emotion")
async def trigger_emotion(request: EmotionRequest):
    ...
    try:
        if hasattr(naga_agent, 'emotional_ai') and naga_agent.emotional_ai:
            ...
            emotion_type = emotion_map.get(request.emotion_type.lower())
            if not emotion_type:
                raise HTTPException(status_code=400, detail="无效的情绪类型")
            
            naga_agent.emotional_ai.add_emotion(emotion_type, request.intensity)
            
            return {"status": "success", "message": f"已触发情绪: {emotion_type.value}"}
        else:
            return {"status": "error", "message": "情绪AI系统未启用"}
    except Exception as e:
        print(f"触发情绪错误: {e}")
...
@app.get("/emotional/config")
async def get_emotional_config():
    """获取情绪AI配置"""
    try:
        emotional_config = {
            "enabled": config.emotional_ai.enabled,
            "ai_name": config.emotional_ai.ai_name,
            "personality_age": config.emotional_ai.personality_age,
            "proactive_enabled": config.emotional_ai.proactive_enabled,
            "vision_enabled": config.emotional_ai.vision_enabled,
            "audio_enabled": config.emotional_ai.audio_enabled,
            "screen_enabled": config.emotional_ai.screen_enabled,
            "file_enabled": config.emotional_ai.file_enabled,
            "auto_exploration": config.emotional_ai.auto_exploration
        }
        return {"status": "success", "data": emotional_config}
    except Exception as e:
        print(f"获取情绪配置错误: {e}")
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"获取情绪配置失败: {str(e)}")
